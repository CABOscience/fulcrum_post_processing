...
def create_record_from_json(record_raw):
  ...
  if 'form_id'            in record_raw:
    ...
    RformName = FO.get_formName_from_formid(RformId)
  ...
  if not RID and not RformId:
    ...
    is_form_project_enabled = FO.get_projects_enabled_status(record.form_id)
    if is_form_project_enabled:
      record.project_name = PO.get_project_name_from_id(record.project_id)
      if not record.project_name:
        ...
        LO.l_debug('Form id {} (name: {}), The record id {} has an associated project name with a project name and it has the project id {}.'.format(record.form_id, record.form_name,record.ID,record.project_id))
    else:
...
def get_records_from_list(listRecords):
  records = Records()
  for record_raw in listRecords:
    record = create_record_from_json(record_raw)
...
def backup_records_versions(form,recs):
  ...
  for record in recs.records[:]:
    ...
    if recordsHistory:
      ...
      recordsHistoryTemp = get_records_from_list(recordsHistory)
