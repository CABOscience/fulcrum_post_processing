...
class Form(object):
  ...
  def set_KeysTypes(self):
    if len(self.dictKeysTypes)<1:
      dictKeysTypes = {}
      search_for_types_form_recu(dictKeysTypes,self.elements)
      self.dictKeysTypes = dictKeysTypes
...
# Forms sub functions
# Recursive search for keys to dataname
def search_for_types_form_recu(dictKeysTypes,info):
  if isinstance(info, list):
    for v in info:
      if v:
        search_for_types_form_recu(dictKeysTypes,v)
  elif isinstance(info, dict):
    if 'elements' in info:
      search_for_types_form_recu(dictKeysTypes,info['elements'])
